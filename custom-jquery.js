$(document).ready(function () {
  ...
  document.body.style.visibility = "visible";
    ...
    adClas = $('.more-comments .user-comment[class="user-comment"]').addClass("desi");
    let sizeItems = $(".more-comments .user-comment").length,
        comentItem = 0;
    $(".more-comments .desi:lt(" + comentItem + ")").fadeIn();
    $(".more-comments-btn").click(function () {
        comentItem = comentItem + 20 <= sizeItems ? comentItem + 20 : sizeItems;
        $(".more-comments .desi:lt(" + comentItem + ")").addClass("comShow");
        let answer = $(".more-comments").find(".admin-response");
        if (answer.prev().hasClass("comShow")) {
            $(".comShow").next(".admin-response").fadeIn().addClass("comShow");
        }
        $(".more-comments .desi:lt(" + comentItem + ")").fadeIn();
        let fulsize = $(".more-comments .comShow").length;
        if (fulsize == sizeItems) {
...
function disableSubmit() {
    ...
    if (submitBtn.disabled) {
        ...
        alert(
            "Ø§Ù…Ú©Ø§Ù† Ø®Ø±ÛŒØ¯ Ø§Ø² Ø³Ø§ÛŒØª Ù‚Ù… ØªÛŒÚ©ØªØŒ Ù…Ø³ØªÙ„Ø²Ù… Ù¾Ø°ÛŒØ±Ø´ Ù‚ÙˆØ§Ù†ÛŒÙ† Ù…ÛŒØ¨Ø§Ø´Ø¯."
        );
    }
}
...
$(".favorite-box").click(function () {
    ...
    if ($(this).closest(".favorite-box").is(".active")) {
        ...
        $.cookie("datafavorites", JSON.stringify(favorite), { expires: 365, path: "/" });
